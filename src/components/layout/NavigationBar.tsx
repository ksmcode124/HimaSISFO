  ...
  return (
    <div className={`fixed z-99999 w-full justify-center h-fit transition-all ease-in-out pt-5 ${scrolled ? 'duration-500 md:pt-0' : 'md:pt-5 duration-500'}`} aria-label="wrapper">
      <header
        ...
        role="banner"
      >
        <Glass preset='custom' className={`${!isMobile ? "bg-[#AFAFAF]/80 md:bg-[#101D2F]/50" : 'bg-transparent'} ${scrolled ? 'duration-700 rounded-t-0 rounded-b-[20px]' : 'rounded-t-[40px] rounded-b-[40px] duration-700'}`} disabled={isMobile}>
          <nav
            ...
            <ul className='flex flex-col items-end justify-between h-full overflow-hidden'>
              <div className="px-6 py-10 gap-y-5 flex flex-col">
                ...
                {items.map((item) => {
                  const isActive = item.href === '/' ? pathname === '/' : (pathname === item.href || pathname.startsWith(item.href))
                  return (
                    <li key={item.href} className="px-3 w-50 flex justify-center">
                      <Link
                        href={item.href}
