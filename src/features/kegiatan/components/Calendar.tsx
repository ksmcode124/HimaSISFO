  ...
  return (
    <div className={clsx(className, "w-full py-4 px-2 md:px-4 bg-white rounded-lg shadow")}>
      ...
      <div className="flex items-center justify-between my-5">
        <h2 className="text-[16px] md:text-[24px] font-bold text-[var(--color-dark-blue)]">
          {formatMonthName(month)}, {formatYear(year)}
        </h2>
        <div className="flex border-3 border-[var(--color-dark-blue)] rounded">
          <button onClick={goPrevMonth} className="p-1 border-r-3">
            <ChevronLeft />
          </button>
          <FilterComp
            type="calendar"
          ...
          />
          <button onClick={goNextMonth} className="p-1 border-l-3">
            <ChevronRight />
          </button>
        </div>
      </div>
      ...
      {/* Header Hari */}
      <div className="grid grid-cols-14 text-white text-sm font-medium border border-white">
        {WEEK_DAYS.map((day, i) => (
          <div
            key={day}
            className="bg-[#6482A6] pl-1 md:pl-2 py-1 uppercase col-span-2 text-xs md:text-sm"
      ...
      <div className="grid grid-cols-14 grid-rows-20">
        {daysGrid.map(({ day, isCurrentMonth, dateObj }) => {
          ...
          const dayEvents = sortEventsByDay(eventsByDate[key] ?? [], dateObj);
          // console.log({ key, dayEvents });
          return (
            <div
              key={key}
              className={clsx(
            ...
            >
              ...
              <div className="h-fit pl-1 md:pl-2 pt-1 md:pt-2 row-span-1">
                <div
                  className={clsx(
                    "font-medium text-[var(--color-dark-blue)] text-xs md:text-sm",
                        ...
                        {dayEvents.map((ev, i) => {
                          ...
                          return (
                            <div
                              ...
                              className={clsx(
                                "absolute left-0 right-0 h-[25px] md:h-[35px] rounded-[5px]",
                            ...
                            >
                              {isTopCard && (
                                <span className="text-white font-bold text-xs md:text-sm">+{dayEvents.length}</span>
                              )}
                            </div>
                          );
                        })}
                        ...
                        if (Number(toDateKey(ev.start).split("-").pop()) === day) {
                          return (
                            <div
                              key={`${ev.id}-${key}`}
                              className={clsx(
                              "relative flex items-center px-2 h-[15px] md:h-[28px] rounded-[4px]",
                              "text-xs md:text-sm font-bold text-white cursor-pointer shadow-sm",
                              "truncate whitespace-nowrap overflow-hidden -translate-y-[2px]",
                              ev.type === "Hima" && "bg-gradient-to-r from-[#1B3C53] to-[#456882]",
                              ev.type === "Beasiswa" && "bg-gradient-to-r from-[#7F1D1D] to-[#DC2626]",
                              ev.type === "Lomba" && "bg-gradient-to-r from-[#CA8A04] to-[#EAB308]"
                            )}
                          ...
                          >
                            <span className="truncate w-full">{ev.title}</span>
                          </div>
                        )
                      }})}
                    </div>
